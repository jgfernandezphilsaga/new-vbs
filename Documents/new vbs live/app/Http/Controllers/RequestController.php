...
public function select_approvers(Request $request)
{
    $term = $request->term;
...
// Main query: LEFT JOIN with dispatch_table split
$query = DB::connection('sqlsrv')
    ->table(DB::raw("({$requests->toSql()}) as a"))
    ->mergeBindings($requests)
    ->leftJoin(DB::raw("
        (
            SELECT
                id,
    ...
    "), function($join) {
        $join->on('a.header_id', '=', DB::raw('TRY_CAST(b.split_id AS INT)'));
    })
    ->select(
        'a.header_id',
        DB::raw('MAX(a.reference_no) as reference_no'),
        DB::raw('MAX(a.requesting_dept) as requesting_dept'),
        DB::raw('MAX(a.user_fullname) as user_fullname'),
        DB::raw('MAX(a.departure_time) as departure_time'),
        DB::raw('MAX(a.origin) as origin'),
        DB::raw('MAX(a.destination_to) as destination_to'),
        DB::raw('MAX(a.trip_type) as trip_type'),
        DB::raw('MAX(a.status) as status'),
        DB::raw('MAX(b.dispatch_reference) as dispatch_reference'),
        DB::raw('MAX(b.id) as dispatch_id')
    )
    ->groupBy('a.header_id');
...
if ($header->status == '1007') {
    ...
    return [
        'dispatch_reference' =>$header->dispatch_reference,
   ...
   public function save_process(Request $request)
{
    ...
    ->join(DB::raw("
        (
            SELECT *,
                   LTRIM(RTRIM(value)) AS split_id
            FROM dispatch_table
            CROSS APPLY STRING_SPLIT(header_id, ',')
            WHERE id = ".(int)$id."
        ) as b
    "), function ($join) {
        $join->on(DB::raw('a.header_id'), '=', DB::raw('TRY_CAST(b.split_id AS INT)'));
    })
    ->first();
    ...
    public function post_to_api($request_header_id)
    {
        ...
        $params = [
            ...
            'name'           => $name,
            'approver_id'          => $vbs_user->approver_id,
            'approval_url'   => null,
     ...
     public function post_dispatch_api($id)
    {
        ...
        $params = [
            ...
            'sourceapp'      => 'Vehicle Booking System',
            'sourceurl'      => request()->root() . '/review_dispatcher/' . $id,
    ...
    public function show_request_details_dispatcher($id_header)
    {
     $dispatch_header = DB::table('dispatch_table')->where('id', $id_header)->first();
        if (!$dispatch_header) abort(404, 'Dispatch not found');

        $header_ids = collect(explode(',', (string) $dispatch_header->header_id))
            ->map(fn($v) => (int) trim($v))
            ->filter()
            ->values()
            ->all();
        ...
        if (empty($header_ids)) {
            abort(404, 'No header IDs found for this dispatch');
        }

        $request_headers = RequestHeader::whereIn('id', $header_ids)
            ->orderBy('id')
            ->get();

        $details_by_header = RequestDetail::whereIn('request_header_id', $header_ids)
            ->where('is_removed', 0)
            ->get()
            ->groupBy('request_header_id');

        return view('admin.reports.request-detail_dispatcher', compact('request_headers', 'details_by_header'));
